...
export default function PolicePortal({ user, onLogout }) {
  ...
  const fetchData = async () => {
    const res = await axios.get('https://missing-person-case-backend-1.onrender.com/api/people');
    setPeople(res.data);
  };
  ...
  const updateStatus = async (id) => {
    await axios.put(`https://missing-person-case-backend-1.onrender.com/api/people/${id}`, { status: 'Found' });
    fetchData();
  };

  const deleteCase = async (id) => {
    if (window.confirm("Permanently delete?")) {
      await axios.delete(`https://missing-person-case-backend-1.onrender.com/api/people/${id}`);
      fetchData();
    }
  };
            ...
            <div className="card p-3 h-100">
              <div className="row g-0">
                <div className="col-4 col-sm-3">
                  <img src={p.photo ? `https://missing-person-case-backend-1.onrender.com/uploads/${p.photo}` : "https://via.placeholder.com/100"} 
