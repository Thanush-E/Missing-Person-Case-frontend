...
export default function PeoplePortal({ user, onLogout }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post('https://missing-person-case-backend-1.onrender.com/api/people', formData);
      alert("Case Reported");
          ...
          {people.map(p => (
            <div className="col-md-6 mb-3" key={p._id}>
              <div className="card p-3 d-flex flex-row align-items-center">
                <img src={p.photo ? `https://missing-person-case-backend-1.onrender.com/uploads/${p.photo}` : "https://via.placeholder.com/80"} 
